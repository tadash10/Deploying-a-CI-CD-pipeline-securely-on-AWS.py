...
def check_and_create_iam_role(role_name, assume_role_policy_document, policy_document, role_description):
    try:
        ...
        if e.response['Error']['Code'] == 'NoSuchEntity':
            ...
            logger.error(f"Error checking or creating IAM role: {e.response['Error']['Message']}")
            raise

def check_and_create_codebuild_project(project_name, role_arn, s3_bucket):
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidInputException':
            ...
            logger.error(f"Error checking or creating CodeBuild project: {e.response['Error']['Message']}")
            raise

def check_and_create_codepipeline():
    try:
        ...
        if e.response['Error']['Code'] == 'PipelineNotFoundException':
            ...
            logger.error(f"Error checking or creating CodePipeline: {e.response['Error']['Message']}")
            raise

def main():
    try:
        # Create S3 bucket and set policies
        check_and_create_s3_bucket(BUCKET_NAME)
            ...
            "Statement": [
                {
                    ...
                    "Resource": f"arn:aws:codepipeline:{AWS_REGION}:{sts_client.get_caller_identity()['Account']}:pipeline/{PIPELINE_NAME}"
                }
            ]
        }

        # Create IAM roles with custom policies
        codebuild_role_arn = check_and_create_iam_role(
            CODEBUILD_ROLE_NAME,
            json.dumps({
                "Version": "2012-10-17",
                "Statement": [
        ...
        pipeline_role_arn = check_and_create_iam_role(
            PIPELINE_ROLE_NAME,
            json.dumps({
                "Version": "2012-10-17",
                "Statement": [
        ...
        check_and_create_codebuild_project("MyCodeBuildProject", codebuild_role_arn, BUCKET_NAME)

        # Create CodePipeline
        check_and_create_codepipeline()

    except ClientError as e:
        logger.error(f"Script failed with AWS ClientError: {e.response['Error']['Message']}")
